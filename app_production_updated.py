...
def extract_json(text: str) -> dict:
    ...
    - LaTeX backslashes inside JSON strings, such as \( \[ \sum \text
    """
    if not text:
    ...
    # Second attempt: Gemini may output raw LaTeX backslashes inside JSON strings.
    # JSON only allows these escapes: \" \\ \/ \b \f \n \r \t \uXXXX
    # This escapes illegal ones like \(, \[, \sum, \text, \, etc.
...
- Every backslash inside JSON strings MUST be escaped.
- Correct JSON inline math string example: "\\\\(F_{{CJ}}\\\\)"
- Correct JSON display equation string example: "\\\\[ \\\\sum F_x = 0 \\\\]"
- Incorrect JSON string example: "\\(F_{{CJ}}\\)"
- Incorrect JSON string example: "\\[ \\sum F_x = 0 \\]"
- Never output HTML.
- Never output MathJax HTML.
- Never output <mjx-container> tags.
- Only use plain text and LaTeX delimiters: \\\\(...\\\\) and \\\\[...\\\\].
"""
